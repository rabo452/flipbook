...
def get_flip_book_info(flip_book_id, domain):
    ...
    flip_book_obj['links'] = [list(set([link for link in re.findall(r'https?:\/\/\S+\b', text_page) if len(link) > 12])) for text_page in flip_book_obj['text_pages']]
    if row.external_download == "true":
        flip_book_obj['external_file_link'] = domain + url_for('files', filename = (f'{row.filesDirectoryID}/external_file/' + os.listdir(os.path.join(FLIP_BOOK_FILES_DIRECTORY, f'{row.filesDirectoryID}/external_file/'))[0]))
    else:
...
def get_user_flip_books(user_id, domain):
    ...
    for row in db.engine.execute('SELECT * FROM `flip_book` WHERE `userID` = ?', [user_id]):
        try:
            ...
            if not row[3]: flip_book['password'] = 'False'
            else: flip_book['password'] = row[3]

            flip_book['images'] = get_flip_book_images(str(flip_book['filesDirectoryID']))
            flip_book['external_file'] = url_for('files', filename = (f'{row[1]}/external_file/' + os.listdir(os.path.join(FLIP_BOOK_FILES_DIRECTORY, f'{row[1]}/external_file/'))[0]))
            flip_books.append(flip_book)
        except: continue # files can be deleted by admin or server, and it'd raise error
...
def upload_file_to_server(file_data, logo_file_data, brand, external_download, disable_right_click, file_extension = None, google_pdf_url = '', password = '', user_id = 1):
    ...
    try:
        ...
        if file_extension in ['jpg', 'png', 'jpeg']:
            ...
            if generated_pdf: os.remove(file_path) # delete the denerated pdf from external file
        ...
        db.engine.execute('INSERT INTO `flip_book` (`filesDirectoryID`, `views`, `password`, `userID`, `brand`, `external_download`,`flip_book_id`, `disable_right_click`) VALUES (?,?,?,?,?,?,?,?)',
...
def change_uninque_directory_id(current_id):
    ...
    db.engine.execute('UPDATE `current_directory_id` SET `unQueeFilesDirectoryID` = ? WHERE `unQueeFilesDirectoryID` = ?',
