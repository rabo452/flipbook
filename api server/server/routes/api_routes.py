...
from server.api.users import create_new_user, get_user_id_by_token, get_token_by_user_info, has_email_account, generate_new_password_user, find_user, activate_user, send_confirm_email, send_password_email
from server.api.flip_book import upload_file_to_server, has_password_flip_book, is_right_password_flip_book, has_flip_book, get_flip_book_info, increase_flip_book_views
...
def api_create_flip_book():
    try:
        ...
        if google_docs_link != '':
            page_id = upload_file_to_server(file_data, logo_file_data, brand, external_download, disable_right_click, None, google_docs_link, password = password, user_id = user_id)
        ...
        for type in allowed_file_types:
            if type in request.files['file'].content_type:
                ...
                if 'vnd.openxmlformats-officedocument' in type:
                    page_id = upload_file_to_server(file_data, logo_file_data, brand, external_download, disable_right_click, 'docx', password = password, user_id = user_id)
                else:
                    page_id = upload_file_to_server(file_data, logo_file_data, brand, external_download, disable_right_click, type, password = password, user_id = user_id)
                return json.dumps({'page_id': page_id})
